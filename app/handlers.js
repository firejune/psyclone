...
function installDarwin() {
  ...
  // handler at runtime.
  app.setAsDefaultProtocolClient('muki');
...
function installWin32() {
  ...
  const iconPath = path.join(process.resourcesPath, 'app.asar.unpacked', 'static', 'muki.ico');

  registerProtocolHandlerWin32('muki', 'URL:Muki Arhives URL', iconPath, process.execPath);
  registerFileHandlerWin32('.muki', 'com.firejune.muki', 'Muki Archives', iconPath, process.execPath);
...
function uninstallWin32() {
  ...
  unregisterProtocolHandlerWin32('muki', process.execPath);
  unregisterFileHandlerWin32('.muki', 'com.firejune.muki', process.execPath);
...
function installLinux() {
  ...
  const STATIC_PATH = path.join(app.getAppPath(), 'assets');

  installDesktopFile();
  installIconFile();

  function installDesktopFile() {
    const templatePath = path.join(STATIC_PATH, 'linux', 'muki.desktop');
  ...
  function installIconFile() {
    const iconStaticPath = path.join(STATIC_PATH, 'muki.png');
